...
export class NumberboxVisual extends ValuedOnePointer {
	...
	render() {
		...
		if (lastValue || lastValue === 0) {
			let roundToZero = Engine.getAttribute(this.primitive!, "RoundToZero");
			let roundToZeroAtValue = -1;
			if (roundToZero === "true") {
				roundToZeroAtValue = Number(Engine.getAttribute(this.primitive!, "RoundToZeroAtValue"));
			...
			let number_length = JSON.parse(Engine.getAttribute(this.primitive!, "NumberLength"));
			let number_options = {
	...
	get targetID() {
		return Number(Engine.getAttribute(this.primitive!, "Target"));
	}
	set targetID(newTargetID) {
		Engine.setAttribute(this.primitive!, "Target", `${newTargetID}`);
		this.render();
	}
	afterNameChange() {
		this.setSelectionSizeToText();
	...
	setColor(color: string) {
		...
		let frameColor = Engine.getAttribute(this.primitive!, "HideFrame") === "true" ? "transparent" : color;
